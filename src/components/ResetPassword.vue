...
onMounted(() => {
  // Retrieve token from query params: /reset-password?token=XYZ
  token.value = route.query.token || ''
  if (!token.value) {
    errorMessage.value = 'Invalid reset password request. The password reset token is missing.'
  }
})
...
async function handleSubmit() {
  ...
  if (!token.value) {
    errorMessage.value = 'Cannot reset password: reset token is missing.'
    return
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Reset-password request failed with status ${response.status}:`, data)
      errorMessage.value = data.detail || 'Failed to reset password. The token may be expired or invalid.'
      return
    }
    ...
    console.error('Network error or exception during reset-password request:', error)
...
© 2026 llms.txt generator
      ...
      <div class="branding-content">
        ...
        <p class="branding-description">
          Create a new strong password to secure<br />
          your llms.txt generator account.
        </p>
      </div>
    </div>
    ...
    <div class="reset-form-panel">
      <div class="reset-form-container">
        ...
        <form v-if="token" class="reset-form" @submit.prevent="handleSubmit">
          <!-- New Password Field -->
          ...
          <div class="input-group">
            <label class="input-label" for="reset-confirm-password">Confirm New Password</label>
            <div class="input-wrapper">
              <svg class="input-icon" width="18" height="18" viewBox="0 0 24 24" fill="none">
                <rect x="3" y="11" width="18" height="11" rx="2" ry="2" stroke="currentColor" stroke-width="1.5"/>
                <path d="M7 11V7a5 5 0 0110 0v4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>
